...
@section('conteudo')
    <main class="h-screen flex flex-col overflow-y-auto px-4">
        <div class="mt-16 mb-2 flex-1 w-full px-6 bg-white rounded-lg shadow-lg p-6">
            ...
            <form class="space-y-1" action="{{ route('servidores.servidor_dependentes_update') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @include('servidores.dependentes.edit_form')
            </form>
        </div>
    </main>
@endsection
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const alert = document.getElementById('toast-success') || document.getElementById('toast-danger') || document.getElementById('toast-info');
            if (alert) {
                setTimeout(() => {
