...
@section('conteudo')
    <main class="h-screen flex flex-col overflow-y-auto px-4">
        <div class="mt-16 mb-2 flex-1 w-full px-6 bg-white">
            ...
            <form class="space-y-1" action="{{ route('servidores.servidor_dependentes_store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @include('servidores.dependentes.create_form')
            </form>
        </div>
    </main>
@endsection
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const showConfirmation = @json(session('showConfirmation'));
            const message = @json(session('message'));
            const dependenteId = @json(session('dependenteId'));

            if (showConfirmation) {
                if (confirm(message)) {
                    window.location.href = `/servidores/dependentes/reativar_dependente/${dependenteId}`;
                }
            }
