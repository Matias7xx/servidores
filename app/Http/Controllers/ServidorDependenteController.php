...
class ServidorDependenteController extends Controller
{
    ...
    public function store(StoreDependenteRequest $request)
    {
        ...
        // Processa o upload do anexo e salva como 'documento'
        if ($request->hasFile('anexo')) {
            $file = $request->file('anexo');
            $filename = uniqid() . '_' . time() . '.' . $file->getClientOriginalExtension();
            $path = $file->storeAs('public/doc_dependentes', $filename);
            $validated['documento'] = $filename;  // ✅ Salva como 'documento', não 'anexo'
        }

        // Processa o histórico
        $historico = [];
        if (isset($validated['historico']) && $validated['historico'] != "") {
            $historico = json_decode($validated['historico'], true);
        }

        $novaAtualizacao = [
            "usuario_matricula" => $validated['matricula'] ?? auth()->guard('web')->user()->matricula,
            "data" => now(),
        ...
        $validated['servidor_matricula'] = $validated['matricula'] ?? auth()->guard('web')->user()->matricula;
        $validated['status'] = 'A';
        ...
        unset($validated['anexo']); // Remove 'anexo' se veio da validação

        // Verifica se já existe
        $dependente = ServidorDependente::where('cpf', $validated['cpf'])
            ->where('servidor_matricula', $validated['servidor_matricula'])
            ->first();
    ...
    public function edit($id)
    {
        ...
        $servidorDependente = ServidorDependente::findOrFail($id);
            ...
            return response()->json([
                'success' => true,
                'data' => [
                    'servidorDependente' => $servidorDependente,
                    'user' => $user
                ]
            ]);
    }
